 ...
 *  특성과 참조 관계도 포함.
 */
// static은 class 소속 비정적은 객체 소속
class Car23{
   CarName name;               
   String color;   
   
   Car23(String aModel, int aYear, String aColor){
      name =new CarName(aModel,aYear);				//  CarName 타입의 name 변수에 CarName 타입의 새로운 객체 생성 
      color=aColor;
   }
...
//      void outColor() {
//         System.out.println(color);      // CarName은  Car클래스 내부적으로 정적멤버이므로 정적멤버만 참조 가능.
//      }
   }
   
   void outInfo() {
      System.out.printf("모델=%s, 년식=%d, 색상=%s\n", name.model, name.year, color);		// CarName 타입의 name 변수 객체 속의 model 접근 
   }
}

public class ClassMain{
   public static void main(String[] args)   {      
      Car23 pride=new Car23("프라이드",2005,"파랑");
...
//      System.out.println(Car.name.model);   	 // name은 Car객체에 소속된  CarName정적클래스 타입의 비정적 멤버변수일뿐-
      ...
      Car23.CarName grandeur=new Car23.CarName("그랜다이져",2009);   	// 정적이너클래스는 주 클래스에 종속 되어 있는 하나의 -
                                                 					// 타입이자 독립적인 클래스임에 유의.
      System.out.printf("모델=%s, 년식=%d\n", grandeur.model, grandeur.year);
      
      
      Car23 carnival=new Car23("카니발",2011,"노랑");
